...
- **Clipboard:** paste (Ctrl+V, or Android's long-press **Paste**) anywhere in the app — text or an image — and the shared clipboard panel opens bottom-right and syncs it to the other device. The 📋 button in the corner opens the panel any time; it also pops open when the other device sends something over. **Copy text** / **Copy image** put the content back on the system clipboard, **Download** saves an image as a file. Dropping an image on the desktop drop zone also goes to the clipboard (documents still queue as files). Whichever side changed most recently wins when a device reconnects after being offline.
